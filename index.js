...
const UUID = process.env.UUID || '81e1665d-0a28-4297-9587-61e3ffd3a831';  // 在不同的平台运行了v1哪吒请修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'adk.umika.cc.cd';           // argo固定隧道域名,留空即使用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiMjI0OWIxNGM3ZDZhNmUyOTg3ZTQ1ZTUyYWY0YTAwMGUiLCJ0IjoiNGM3ZjNkNjktNjUwNi00YzhjLWI3OGItMmI2NWZjMzFjM2U4IiwicyI6Ik1tUTRORGMzWlRRdE5qSmxPUzAwTkdZeUxUaGhOekV0TWpVME9HTXhOamc1TVRjMCJ9';               // argo固定隧道token或json,留空即使用临时隧道
const ARGO_PORT = process.env.ARGO_PORT || 8060;             // argo固定隧道端口,使用token需在cloudflare控制台设置和这里一致，否则节点不通
const S5_PORT = process.env.S5_PORT || '';                   // socks5端口，支持多端口的可以填写，否则留空
const TUIC_PORT = process.env.TUIC_PORT || '';               // tuic端口，支持多端口的可以填写，否则留空
const HY2_PORT = process.env.HY2_PORT || '3078';                 // hy2端口，支持多端口的可以填写，否则留空
const ANYTLS_PORT = process.env.ANYTLS_PORT || '';           // AnyTLS端口，支持多端口的可以填写，否则留空
const REALITY_PORT = process.env.REALITY_PORT || '3078';         // reality端口，支持多端口的可以填写，否则留空
